...
//		mainTable.scrollTo(translations.size());
	...
	@FXML
	void exportToMP3(ActionEvent event) throws IOException, InterruptedException {
		Runtime.getRuntime().exec("mkdir outMP");
//		ProcessBuilder builder = new ProcessBuilder();
//		builder.command("sh", "-c","gtts-cli", "hola", "--lang", "es", "--slow", "--output", "outMP/hola.mp3");
//		Process process = builder.start();
//		int exitCode = process.waitFor();
//		System.out.println ("exit: " + exitCode);
//		Process p = Runtime.getRuntime().exec("gtts-cli \"hola\" --lang es --slow --output outMP/hola.mp3");

//		for (TranslationRow item : mainTable.getItems()) {
//
//		}
		if(mainTable.getItems().size()>50) {
			return;
		}
		String json = mainTable.getItems().stream()
				.map(item -> String.format("\\\"%s\\\":\\\"%s\\\"", item.getWord().replaceAll(" ", "_"), item.getTranslation().replaceAll(" ", "_")))
				.reduce((a, b) -> a + "," + b).get();
//		Process p = Runtime.getRuntime()
//				.exec("python3 src/main/resources/exportToMP3.py {\"hola\":\"привіт\"} outMP3/dict.mp3");
		System.out.println("json="+json);
		System.out.println("command="+String.format("python3 src/main/resources/exportToMP3.py \"{%s}\" %s", json, "outMP3/dict.mp3"));
		Process p = Runtime.getRuntime()
				.exec(String.format("python3 src/main/resources/exportToMP3.py \"{%s}\" %s", json, "outMP3/dict.mp3"));
		p.waitFor();
		System.out.println("exit: " + p.exitValue());

		logStream(p.getInputStream(), "output");
		logStream(p.getErrorStream(), "error");
		p.destroy();
//		System.out.println(
//				"gtts-cli \"" + item.getWord() + "\" --lang es --slow --output outMP/" + item.getWord() + ".mp3");
//		p = Runtime.getRuntime().exec("/bin/sh -c gtts-cli \"" + item.getWord() + "\" --lang es --slow --output outMP/"
//				+ item.getWord() + ".mp3");
//		p.waitFor();
//		System.out.println("exit: " + p.exitValue());
//		p.destroy();
	...
	private void logStream(InputStream errorStream, String type) throws IOException {
		String s;
		BufferedReader stdError = new BufferedReader(new InputStreamReader(errorStream));
		System.out.println(String.format("Here is the standard %s of the command (if any):\n", type));
